...
mask_path = os.path.join(BASE_DIR, 'input_data/region_bounds.geojson')
reproj_bounds_path_4326 = os.path.join(BASE_DIR, 'input_data/region_bounds_4326.shp')
reproj_bounds_path_4269 = os.path.join(BASE_DIR, 'input_data/region_bounds_4269.shp')
...
reproj_mask_nalcms_crs = os.path.join(BASE_DIR, 'input_data/region_bounds_nalcms_crs.shp')
...
if not os.path.exists(reproj_glhymps_fpath):
    ...
    if not os.path.exists(reproj_mask_glhymps_crs):
        mask = gpd.read_file(mask_path).to_crs(glhymps_original_wkt)
        mask.to_file(reproj_mask_glhymps_crs)
    
    mask = gpd.read_file(reproj_mask_glhymps_crs)
    # print(mask.bounds.values[0])
